...
def cargar_config(path_config: Path) -> dict:
    ...
    default = {
        ...
        "categorias_peliculas": ["Acción", "Comedia", "Drama", "Terror", "Ciencia Ficción", "Romance", "Animación", "Documental", "Otros"],
        "nacionalidades_novelas": ["Mexicana", "Colombiana", "Turca", "Brasileña", "Chilena", "Argentina", "Española", "Estadounidense", "Otra"]
    }
    try:
...
def inicializar_db(db_path: Path):
    ...
    c = conn.cursor()
    c.execute('''CREATE TABLE IF NOT EXISTS clasificacion (
...
def obtener_clasificacion(nombre_base: str, db_path: Path):
    conn = sqlite3.connect(str(db_path))
    c = conn.cursor()
    c.execute("SELECT tipo, categoria, nacionalidad FROM clasificacion WHERE nombre=?", (nombre_base,))
    res = c.fetchone()
    if res:
        conn.close()
        return res
    ...
    for p in palabras:
        if len(p) > 3:
            c.execute("SELECT tipo, categoria, nacionalidad FROM palabras_clave WHERE palabra=?", (p,))
            r2 = c.fetchone()
            if r2:
                conn.close()
                return r2
...
def guardar_clasificacion_en_db(nombre_base: str, tipo: str, categoria: str, nacionalidad: str, db_path: Path):
    conn = sqlite3.connect(str(db_path))
    c = conn.cursor()
    c.execute("INSERT OR REPLACE INTO clasificacion VALUES (?, ?, ?, ?)", (nombre_base, tipo, categoria, nacionalidad))
    for palabra in re.findall(r'\w+', nombre_base.lower()):
        if len(palabra) > 3:
            c.execute("INSERT OR IGNORE INTO palabras_clave VALUES (?, ?, ?, ?)", (palabra, tipo, categoria, nacionalidad))
    conn.commit()
    conn.close()
...
# -------------------------------
def clasificar_por_lote(pendientes, cfg, db_path: Path):
    # pendientes: lista de nombres_base a clasificar
    resultados = {}  # nombre_base -> (tipo, categoria, nacionalidad)
    idx = {"i": 0}
    ...
    root = tk.Tk()
    root.title("Clasificar contenido")
    ...
    subtipo_var = tk.StringVar(value="audio")  # para música
    omitir_todos = {"flag": False}

    label_titulo = ttk.Label(root, text="", font=("Arial", 11, "bold"))
    label_titulo.pack(pady=10)

    label_prog = ttk.Label(root, text=f"0/{total}")
    label_prog.pack()

    frame_tipo = ttk.LabelFrame(root, text="Tipo de contenido")
    frame_tipo.pack(fill="x", padx=10, pady=6)
    for t in ["serie", "pelicula", "novela", "musica", "show"]:
        ttk.Radiobutton(frame_tipo, text=t.capitalize(), variable=tipo_var, value=t).pack(side="left", padx=5)

    frame_categoria = ttk.LabelFrame(root, text="Categoría (Películas)")
    frame_categoria.pack(fill="x", padx=10, pady=6)
    combo_categoria = ttk.Combobox(frame_categoria, textvariable=categoria_var, values=cfg["categorias_peliculas"], state="readonly")
    combo_categoria.pack(fill="x", padx=8, pady=6)

    frame_nacionalidad = ttk.LabelFrame(root, text="Nacionalidad (Novelas)")
    frame_nacionalidad.pack(fill="x", padx=10, pady=6)
    combo_nacionalidad = ttk.Combobox(frame_nacionalidad, textvariable=nacionalidad_var, values=cfg["nacionalidades_novelas"], state="readonly")
    combo_nacionalidad.pack(fill="x", padx=8, pady=6)

    frame_subtipo = ttk.LabelFrame(root, text="Subtipo (Música)")
    frame_subtipo.pack(fill="x", padx=10, pady=6)
    combo_subtipo = ttk.Combobox(frame_subtipo, textvariable=subtipo_var, values=["audio", "video"], state="readonly")
    ...
    def guardar_y_siguiente():
        ...
        # Guardar en DB
        guardar_clasificacion_en_db(nombre_base, t, cat, nac, db_path)
        avanzar()
    ...
    ttk.Button(frame_botones, text="Guardar y siguiente", command=guardar_y_siguiente).pack(side="left", padx=6)
    ttk.Button(frame_botones, text="Omitir", command=omitir).pack(side="left", padx=6)
    ttk.Button(frame_botones, text="Omitir todos", command=omitir_todos_fn).pack(side="left", padx=6)

    cargar_actual()
    root.mainloop()

    if omitir_todos["flag"]:
        # No se agregan clasificaciones nuevas
        pass
    return resultados
...
def organizar_simple(ruta: Path, extensiones: list) -> str:
    ...
    for (nombre_serie, temporada), archivos in archivos_por_serie.items():
        nombre_carpeta = nombre_serie if not temporada else f"{nombre_serie} - {temporada}"
        ...
        logs.append(f"Carpeta creada: {nombre_carpeta} | Total archivos: {len(archivos)}")
        logs.append("=" * 50)
...
# -------------------------------
def organizar_clasificar(ruta: Path, db_path: Path, cfg: dict) -> str:
    inicializar_db(db_path)
    extensiones = set(e.lower() for e in cfg["extensiones"])
    archivos = [f for f in ruta.iterdir() if f.is_file() and f.suffix.lower() in extensiones]
    ...
    nuevos = clasificar_por_lote(pendientes, cfg, db_path)
...
def main():
    ...
    cfg = cargar_config(Path(args.config_path) if args.config_path else resource_path("config.json"))
