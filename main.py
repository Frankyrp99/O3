...
def seleccionar_directorio():
    directorio = filedialog.askdirectory(mustexist=True, title="Selecciona una carpeta", initialdir="./")
    if directorio:
...
def ejecutar_script():
    ...
    # Verifica que organizador_core.py esté accesible
    if getattr(sys, "frozen", False):
        script_path = os.path.join(sys._MEIPASS, "organizador_core.py")
    else:
        script_path = resource_path("organizador_core.py")

    if not os.path.exists(script_path):
        messagebox.showerror("Error", "No se encontró 'organizador_core.py'.")
        return
    ...
    for i, directorio in enumerate(carpetas, start=1):
        try:
            # Ejecutar el core por carpeta para mantener el UI responsivo
            cmd = [
                sys.executable,  # Usa el intérprete actual (mejor que "python")
                script_path,
                "--modo", modo,
                "--dir", directorio,
                "--db", db_path,
            ...
            if resultado.returncode != 0:
                resultados.append(f"[FALLO] {directorio}\n{error or salida}\n")
            else:
    ...
    # Muestra un resumen
    resumen = "\n".join(resultados)
    messagebox.showinfo("Resultado", resumen if len(resumen) < 5000 else resumen[:5000] + "\n...\n(Resumen truncado)")
...
# Widgets
label_instrucciones = ttk.Label(ventana, text="Añade carpetas y elige cómo procesarlas:")
label_instrucciones.grid(row=0, column=0, columnspan=3, pady=(12, 6), padx=12, sticky="w")
...
ttk.Radiobutton(frame_modo, text="Solo ordenar (v1.0)", variable=modo_var, value="simple").pack(side="left", padx=8, pady=4)
ttk.Radiobutton(frame_modo, text="Clasificar y ordenar", variable=modo_var, value="clasificar").pack(side="left", padx=8, pady=4)
...
ttk.Button(ventana, text="Agregar Carpeta", command=seleccionar_directorio).grid(row=4, column=0, pady=6, padx=12, sticky="ew")
ttk.Button(ventana, text="Eliminar Carpeta", command=eliminar_seleccion).grid(row=4, column=1, pady=6, padx=6, sticky="ew")
ttk.Button(ventana, text="Ayuda", command=abrir_ayuda).grid(row=4, column=2, pady=6, padx=(6,12), sticky="ew")
...
ttk.Button(ventana, text="Ejecutar", command=ejecutar_script).grid(row=6, column=0, columnspan=3, pady=10, padx=12, sticky="ew")
